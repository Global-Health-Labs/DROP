...
def reorder_groups(worklist, exp_time):
    ...
    for dst_group in np.sort(time_worklist['destination_group'].unique()):
        ...
        sub['group_forward'] = np.append(sub['group'].values[1:], [0]).astype(int)
        sub['group_backward'] = np.append([0], sub['group'].values[:-1]).astype(int)
        # next_df = next_df.append(sub, sort=False)
        next_df = pd.concat([next_df, sub], sort=False)
    ...
    while time_worklist.shape[0] > 0:
        ...
        time_new = pd.concat([time_new, sub], sort=False)
